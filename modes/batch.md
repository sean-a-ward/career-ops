...
Pi conductor
  ...
  │    └─► pi -p worker → report .md + PDF + tracker-line
  │
...
Each worker is a child `pi -p` process with a clean context. The conductor only orchestrates.
...
4. **For each pending URL**:
   ...
   d. Execute via Bash:
      ...
      ```bash
      pi -p \
        --append-system-prompt "$(cat batch/batch-prompt.md)" \
