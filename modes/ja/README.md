...
ほとんどの求人が英語の場合は、デフォルトの `modes/` を使ってください。英語モードでも Pi が日本語の求人を検出すれば自動的に対応しますが、日本市場の細かい慣習までは同じレベルで把握していません。
...
Pi は `modes/` の代わりにこのフォルダのファイルを読み込みます。
...
最初のセッションで Pi にこのフィールドを尊重するよう伝えてください（「`profile.yml` を見て、`language.modes_dir` を設定してある」）。以降、Pi は自動的に日本語モードを使用します。
