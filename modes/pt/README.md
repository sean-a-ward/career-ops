...
Se a maioria das suas vagas é em inglês, fique com os modos padrão em `modes/`. Os modos em inglês funcionam automaticamente quando Pi detecta uma vaga em português — mas não conhecem as particularidades do mercado brasileiro no mesmo nível de detalhe.
...
Diga ao Pi no início da sessão:
...
Pi vai ler os arquivos desta pasta em vez de `modes/`.
...
Lembre o Pi na primeira sessão de respeitar esse campo ("Olha no `profile.yml`, eu configurei `language.modes_dir`"). A partir daí, Pi usa automaticamente os modos em português.
