...
Process multiple job offers in parallel via `pi -p` workers. Each worker runs the full evaluation pipeline (A-F report + PDF + tracker line) autonomously.
...
|------|---------|-------------|
| `--parallel N` | `1` | Number of concurrent `pi -p` workers |
...
2. For each pending offer, it assigns a report number and launches a `pi -p` worker with `batch-prompt.md` as the system prompt (placeholders like `{{URL}}`, `{{REPORT_NUM}}` are resolved).
...
- `pi` CLI in PATH with a configured model
