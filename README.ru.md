...
| **Пакетная обработка** | Параллельная оценка через `pi -p` воркеры |
...
# 5. Персонализируй через Pi
pi   # Открой Pi в директории проекта

# Попроси Pi адаптировать систему:
...
- **Агент**: Pi с кастомными навыками и режимами
