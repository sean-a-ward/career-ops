...
career-ops 具备代理式工作能力：Pi 会用 Playwright 浏览招聘页面，通过推理你的简历与职位描述是否匹配来评估契合度，而不是只做关键词匹配；同时它也会根据每个职位调整你的简历。
...
> **这个系统本来就是设计给 Pi 直接定制的。** modes、职业原型、评分权重、谈判脚本，直接告诉 Pi 要改什么就行。Pi 读取的正是它自己会使用的那些文件，所以它知道该改哪里。
