...
AI-powered job search automation built on Pi: pipeline tracking, offer evaluation, CV generation, portal scanning, batch processing.
...
When using [Pi](https://github.com/badlogic/pi-coding-agent), slash commands are prompt templates defined in `.pi/prompts/`. Pi also loads `.pi/skills/career-ops/SKILL.md` on demand when a career-ops workflow matches the user request.

| Command | Description |
|---------|-------------|
| `/career-ops` | Show menu or evaluate JD/URL with args |
| `/career-ops-pipeline` or `/career-ops pipeline` | Process pending URLs from inbox |
| `/career-ops-evaluate` or `/career-ops oferta` | Evaluate job offer (A-G scoring) |
| `/career-ops-compare` or `/career-ops ofertas` | Compare and rank multiple offers |
| `/career-ops-contact` or `/career-ops contacto` | LinkedIn outreach (find contacts + draft) |
| `/career-ops-deep` or `/career-ops deep` | Deep company research |
| `/career-ops-pdf` or `/career-ops pdf` | Generate ATS-optimized CV |
| `/career-ops-training` or `/career-ops training` | Evaluate course/cert against goals |
| `/career-ops-project` or `/career-ops project` | Evaluate portfolio project idea |
| `/career-ops-tracker` or `/career-ops tracker` | Application status overview |
| `/career-ops-apply` or `/career-ops apply` | Live application assistant |
| `/career-ops-scan` or `/career-ops scan` | Scan portals for new offers |
| `/career-ops-batch` or `/career-ops batch` | Batch processing with parallel Pi workers |
| `/career-ops-patterns` or `/career-ops patterns` | Analyze rejection patterns and improve targeting |
| `/career-ops-followup` or `/career-ops followup` | Follow-up cadence tracker |

**Note:** Pi reads project instructions from `AGENTS.md`, discovers prompt templates from `.pi/prompts/`, and discovers the project skill from `.pi/skills/career-ops/SKILL.md`. All commands share the same evaluation logic in `modes/*.md`.
...
**Exception for batch workers (`pi -p`):** browser automation may be unavailable in print-mode workers. Use HTTP fetch or page text fallback and mark the report header with `**Verification:** unconfirmed (batch mode)`. The user can verify manually later.
...
- Batch in `batch/` (gitignored except scripts and prompt); workers run with `pi -p`
