...
```
batch-input.tsv    →  batch-runner.sh  →  N × pi -p workers
...
Each worker is a headless Pi worker (`pi -p`) that receives the full `batch-prompt.md` as context. Workers produce:
- Report .md
- PDF
- Tracker TSV line
